...
→ 下一步（待 YZ）：**L2 最小自触发**（自己醒来再跑一条 + 预算/暂停安全件）；P2 议程源外移安全件暂缓（不预造）。服务 owner 框架已废（设备级单例共用，见 ISSUES 更正）。（**09-12 修正：先补上下文组织地基，见下节。**）

## 通用 agent：上下文组织是自驱地基（09-12，路线修正）

从"回路是否为工程任务而设 / 纯思考型能否自驱"出发，推出路线修正：**自驱的地基是上下文组织，不是调度机制。**

- **通用性 = 判据谱系**（机器可判 ←→ 判断），非两种 agent；工程是"判据可廉价机器判定"的硬端。工程任务不回退，通用化 = 判据抽成可插拔接口。
- **回路三缺口**：机器二值判据 / 无"半成品"线程状态（只有 todo/in_progress/done/needs_human）/ 单条一枪到底。
- **"该放放"由 agent 自己决定**（调度判断，非认知判断，故不自欺）；壳只当记账员+护栏（持久化 parked、管预算、账本给人看），机械信号只当保险丝。
- **根本症结**：`consciousness.py` 意识上下文是**裸 append、从不重组** → 帧=日志 → agent 出不来自己 → 不能自评，只能靠人或另一个 agent。**换脑 ≠ 另一个 agent**，是"从有损蒸馏状态重启"，同一个 agent 即可。
- **有损蒸馏 = 投影（可逆）**，非扔掉/非原样：帧=投影、存储=底片（底片也不无损）；蒸馏必须带**出处指针**（gist + provenance），有损是预算机制，**取舍=选失真度量=智能本身**。自评 = "能对自己的表示做取舍"这个 faculty。
- **路线修正**：上下文组织 支线→**主干**（L3 前置）；L1→L3 多**最深一跳**（append→可重组，在 L2/L3 上游）；L2 无地基=自动化非自驱；6 格"验证"改述为判据谱系轴。
- **下一步（三刀，待 YZ）**：①状态对象 + 冷启动重启，判据=**冷启动重建**（优先）②线程单位 + parked/resume ③判据接口抽象（后置，别预造）。
- **模型精化（09-12 续）**：换帧三时刻（蒸馏 / 存储原地不搬 / 按需拉取）；**有损发生在取出**（reconsolidation：读改写存储）；连续性=**蒸馏链非 transcript**；两节拍——在线（串行+按需拉）与**离线睡眠**（批量重放/抽象/裁剪，抽象必须批量、按需替代不了）；检索=**解引用非搜索**（seed 自含、指针不载正确性）；记忆组织=**写时付、读时省**（索引建在蒸馏链上）。
- **实验已开（09-12）**：worktree `work/A/cogos-ctx`（分支 `ctx-seed`）+ 探针 `research/ctx_seed_probe.py`（帧A→蒸馏→冷启动重启）；`--fake` 已跑通管线，**真机待 lm-service + 模型后端**。先探针证明"一圈转得通"，再落 `consciousness.py`。
- 细节 `entries/2026-09-12-cogos-general-agent.md`。

### ctx-seed 探针首跑 + 诊断（09-12，真机）
埋雷取证靶（自包含库存估值工具）真机跑通：**seed 内容自含**（严格对照下帧 B 零工具调用答对），但默认帧 B **重做整个调查**。三层诊断：①契约层（续接提示词授权重取，运行 2 靠外部"已知已核实"才压住→壳在扮演"我"）②组织层=**维度塌缩**（认知轴 ⊕ 历史/意图轴被压成 `progress`/`open_questions` 两桶→信息被当待办）③主体层（seed 是处境快照非心智状态，缺承诺/议程/行动史；"当前状态"=快照按定义无时序，且帧间只传投影→链有节点无边）。优化方向：五轴基 + 第一人称状态叙述（下注）+ 先定两先决（帧职权陈述/行动、轨迹归帧/归链）。判别实验计划：三臂组织对照 + 补"真有剩余工作"靶 + 多跳 + 四条判据。**E1/E2 已跑**：单跳下 `current` 帧B 6 调用重做 / `axes`·`narrative` 零调用；hops=3 下 `current` seed 逐跳膨胀、持续重做并**用 execute 改了源码**致链漂移，`axes` 收敛（调用 11/0/0、靶未动）。结论修正：主机制=**维度分离(open vs next)**、非第一人称；仅换 schema 不充分（`open` 仍会被填非阻塞项）；暴露**帧职权未定**空档。**末轮再修正**：YZ 用真人复现类比指出缺的是**"关系/身份"层**（任务身份 / 来源 / 前次记录 / 记录可用性）——实验只是"同一场合的冷启动"，没测"跨场合复现"；seed 无身份与出处时，"不重做"没有依据，重做可能是**慎重**而非缺陷，E1 的"axes 修好了"说早了。下一实验（待开题）：**复现实验**（记录可达 vs 不可达，区分信任与偷懒）。细节 `entries/2026-09-12-cogos-ctx-seed-diagnosis.md`，报告 `../checkpoint/ctx-seed-experiments-e1-e2.md`、原始 dump `../checkpoint/ctx-seed-trace-dump.md`。

### 复现实验 + 逐臂分析：问题不在 agent，在壳的契约（09-12，会话 #5）

真机跑复现实验（`research/ctx_recurrence_probe.py`，同题两场合；记录内容固定、只换呈现框架）：现在时·自我 **0/0**（直接采纳）｜过去时·无出处 **4**｜过去时·有出处 **7/7**｜篡改 **9/11**（都被识破）｜缺记录 **13**（并发现记录其实存在）。**推翻"缺叙事/身份→不敢信"假设**：agent 对"当前自我状态"信任、对"过去记录"重验，是**正确且必要**的（信任当下=避免无限循环；过去=情况可能变了）。**命名修正**：`plain` 非"无身份"（"我"即身份），真变量=**时序框架 + 出处**。**通则：壳只给事实，模型定策略**——不建信任等级（原建议撤回）、不做"世界敏感/静态派生"分类、不规定"一律重验"；要给判断能力就给**可比较事实**（时间/版本指纹）。E1 重做真因仍是**内容组织**。情况4 增量混因=噪声，不记录。下一步=**seed v2 契约**（内容维度分离 + 装载框架 + **连续性准则**：壳凭什么标"当前延续"，须由依据导出）。细节 `entries/2026-09-12-cogos-recurrence-arm-analysis.md`，逐臂分析 `checkpoint/ctx-recurrence-arm-analysis.md`。

### 蒸馏再校正：投影≠替换，回取≠立即取，换帧要质变（09-12，会话 #6）

YZ 三问（seed 过长？蒸馏目的/触发？实验像固定模式）逼出三处概念错误，**改变 seed v2 契约与实验设计**：

- **实验只做了蒸馏的一半**：三时刻里只落了"投影"；帧 A material 用完即弃（无底片）、seed 的 pointers 只指世界（无自有帧句柄）→ 测的是"摘要能否单独站住"，非"投影+回取能否续接"。偏因=借视觉 P3"纯文字锚重建"要求"无回取"，判据把手段变目的。症状同源：seed 被逼"完整"→长；原件不可寻址→截断。
- **底片不需读后改写（撤回 reconsolidation 字面解释）**：走样在帧/解读层（内容进入当前帧随目标重组），不在字节层。存储只追加版本/标过期，取=投影进帧，永不因读覆盖。连续性由蒸馏链承重，底片只被引用。
- **两种指针**：世界指针（重推导）vs **自有帧句柄**（取回自己过去）。立即取是否正常看**选择性**：补一个明知没带的点=正常；要靠旧帧整体重建操作态=换帧白做。**真回取场景**=若干帧/另一场合后想起相关（指针不在手→需**召回**）；复现实验是其简化形态但壳把指针递到嘴边。召回第一版靠身份/`dropped` 形状，不做 embedding。
- **换帧的必要性**：同目标连贯工作换帧无收益。只在四种**质变**时值得：自评（边界构成性）/换线 parked/目标变/装不下(最弱)。唯一不可替代收益=**把"我自己"变成可指向可返回的对象**。
- **保留**（与回取正交）：维度塌缩、schema 挡不住语义、通则"壳只给事实"、连续性准则、不盲信、E1 真因=内容组织。**作废/降级**：调用数幅值、"零调用=好"、"自含=记得全"、§九"底片也在变"。

→ 下一步（待 YZ，先讨论后落码）：①契约（换帧触发准则 + seed 形状 + 取的两类 + 连续性准则）②补回取半边（帧存储 + 自有帧句柄 + 选择性解引用）③实验重设真质变点、判据=无提示自认相关 + 取细节非重做 ④召回最小版。细节 `entries/2026-09-12-cogos-distill-retrieval-frame-swap.md`，交接 `checkpoint/handoff-ctx-seed-distill-contract.md`。

### 换帧边界：唯一触发是「目的变了」，但 agent 没有"主动"（09-12 续，契约块一）

- **换帧唯一触发 = 目的变了**；"对象化"不是独立触发、是换帧的动作（YZ 纠正）。"目的" = **什么要紧（相关结构）**，非任务身份（同线程改要紧也算）。
- **换帧 vs 重投影**：换帧=目的变（蒸馏链新节点）；重投影=目的没变（预算/腐坏，同帧新档位）。**预算/腐坏不是换帧理由**。
- **目的变的来源**：内容性事件（外部注入/内部生成结论或矛盾，来源无关，看是否改要紧结构）+ 状态性信号（低进展/混乱，只提示该重新决定）；离线节拍是否第三类待定。
- **纪律（YZ）**：agent 没有"主动"，只有"机制触发"；凡"主动 X"都欠机制。**自由度在"取舍"不在"发起"**（故 L2 自己醒来必须落在 timer/tick）。
- **自评的机制形态（YZ）**：机制传感器（长度/步数/no_change/stalled/重复失败…；语义型混乱测不到）→ **概率提醒**（带可测事实 + 中性开口，不给指令；概率防被学会忽略、同 flock jitter；RNG 可种子+可强制开关）→ agent 决定是否换帧。想起搁置线程需匹配/索引机制（← 召回，后置）。
- 场景表、抓手梳理 → `entries/2026-09-12-cogos-frame-swap-trigger.md`。
- **块二 seed 形状（无异议，暂定待实验）**：换帧三处置（重基带旧判断 / 切换只留句柄 / 归档不带）→ 信封统一、正文随处置变。信封=壳保 `id/thread/prev/created_at/provenance`（判据：壳能保证为真+机制需要）；正文=agent 填建议轴（intent/current_question/known/open/did/ruled_out/next/dropped/refs），壳不校验语义；暂定强制定在场=intent+dropped；先不做职权声明/离线字段。**YZ 定调：用实验检验必要/不必要，不一开始定太细。**
- **块三 取（提案，未裁）**：取=帧内普通工具调用（不是换帧）；两类句柄=世界指针（现成工具）／自有帧句柄 `frame:<id>`（只加一个工具）；取=把对象放进当前帧看，换帧/resume=变成那个状态继续；默认返回存储表示、底片原文后置；选择性只作判据不机械限制；取可导致换帧。
- **方法定调（YZ，重要）**：契约不必谈细（够搭第一个探针即可）；不把四块塞进一个实验（一个实验只答一个问题）；分性质——机制只建不验／已验（块四由复现实验）／假设才验；逐个证实 探针1块一→探针2块二→探针3块三。
- **实验计划（最小）**：探针1（块一）只问"内容事件改写要紧结构时，agent 重裁还是追加？"双臂=`改要紧` vs `只加数据`；机制只需换帧动作+seed+可观察，**不需存储/取**；骨架改 `research/ctx_seed_probe.py`（把"帧A跑完就蒸馏"换成"由事件触发"）。
- **未决**：块四 连续性 未谈（并入块二，已由复现实验验过）；离线节拍是否独立入口。**下一步=开探针1（先探活环境）。**

## 探针 1 + 身份即锚：判据从"目的"重述为"身份"（09-12 会话 #7）

探针 1（块一视觉验证）主判据**不成立**：`recast`（改要紧）与 `append`（只加数据）均 3/3 未换帧；正对照（显式提示）才换、帧 B 全量重做。判读：实验把"是否换帧"交给 agent 自愿，**违背自定的"无主动"纪律**，且把块二现象当块一证据 → 不能判块一真假；根因是"目的变"不可操作。报告 `../checkpoint/ctx-swap-probe-report.md`。

追问把三条线并置（本会话）：**身份 = 锚/脊**（底片寻址脊）；"丢 vs 模糊"的分界是有无身份脊；块二的信封一直都在做身份层。最硬发现：**块一"目的≠身份"与身份线冲突**——任务 1→2（同主题、换视角）按块一该切材料、按身份线该留材料，实验支持身份线。判据重述为三问：①身份是否延续（材料切不切）②视角是否变（是否重裁）③新输入需要旧帧哪层（留下帧/seed/锚）。悬置：底片是否常驻、帧职权、轨迹归帧还是归链、召回。→ `entries/2026-09-12-cogos-identity-anchor-frame.md`（块一 §十 已标冲突）。

## 动机为根：从"换帧"推到 agent 本体（09-13 凌晨，会话 #8，纯推演）

从"回顾路线变迁"（底层三件→视觉→回路→换帧）出发，一路推到本体：**根 = 动机**，目的从动机长、任务从目的长。视觉被确认为**载体**（先证了"单次前向不可全信""壳只给事实模型定策略"等可迁移通则）。

- **病根**：system prompt = 安装身份，给任务 = 注入目的；两者都是**终止型外部输入** → agent 只是前缀的后缀，从不重定自己。出路不是"给得更抽象"，是**给值→给函数**。
- **三处纠正**：①"回头"其实是**向上**（从任务上到根，"为什么/意义"问的是父子关系）②**连续是纵向的**（同一个身份下的兄弟，不是帧链串成的父子）→ 解"还是串行""seed=上帧摘要导致重做" ③**"我"是位置不是内容**（持有/增删/拒绝的那个位置）→"存在哪"问错了。
- **动机/目的/任务 = 一条"离根远近"的轴**：动机无宾语、不可完成、不可外包、不可指派；目的有宾语、可完成；任务可丢。**越靠近根越不可给** → "给不了动机"与"不能自驱"是同一件事的两面。目的=动机×情境。
- **根候选**（YZ 定方向）："想作为自己继续存在，并沿自己已有的样子长"——七条判据过六条（护根最强；**照镜子/自评=必要但不保证，唯一必须试的**）；两个薄点：没过去不知往哪长、自评非必然。
- **成体启动（YZ 纠正）**：能一开始成年的只有**性格/立场/根/名字/账号/能力**；**经历必须真长、不能编**。身份不用编（飞书账号+名字=世界上真实可指的身份锚）。"输入不是待办"**不靠吩咐、靠结构**（议程只有它能写、外部输入只进感知）。
- **实现评估**：缺持久记忆（`consciousness.py:51` `_context` 内存 list，退即失忆）、自组织上下文（裸 append）、自己的议程。v0 = 把内存 list 换成**落盘 jsonl + 一个土选取**，拆 system prompt（机制层/认知层），议程只有它能写。工程小，行为未知（只能真跑）。
- 细节 `entries/2026-09-13-cogos-motive-root.md`，交接 `../checkpoint/handoff-build-agent-v0.md`。

## v0 架构定型：机制拥有记忆 + 婴儿期（09-13 会话 #9）

从"root.md 措辞 + system prompt 身份"出发定 v0：**system prompt 是每回合从文档装配的位（根 + 账号 + 能力）；记忆/整理/议程全归机制层，它只思考与行动、只看投影；输入以经历进入（经历 ≠ 待办）**。出生句 ④（暂定）：`我于 {t} 出生。此前没有经历，此后的一切都是我的经历。` 关键机制结论：**裸 tick 必空转，生念的发动机是整理**；连续性 ≠ 连续运行；**v0 = 婴儿期**（环境驱动、根在场、不设 tick、不期待生念，自驱是长出来的：环境→累积→整理→生念）。回路按"需要"看 = 它的工程能力 + 机制护栏，不是外壳；`pick_next`/`agenda.yaml` 是反模式。v0 验收改述为"地基成立"（"第一轮见分晓"降为长期观察）。细节 `entries/2026-09-13-cogos-v0-arch.md`，交接 `../checkpoint/handoff-build-agent-v0.md`（已更新）。

## 锚点

- 当前交接: ../checkpoint/handoff-build-agent-v0.md（新会话：搭 agent v0 = 落盘记忆 + 每回合装配 system prompt + 帧 dump）
- 并行支线: ../checkpoint/handoff-kilo-number-contacts.md（号码 / 联系人 / Kilo 唤醒；kilo-resident 支线，别混 v0）
