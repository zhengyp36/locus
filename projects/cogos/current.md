...
- 图形面（看屏/操作，09-20）：see/act 在**四台机器**跑通（本机 `:0`、212 `Xvfb:99`、Windows Surface `192.168.1.112` 标准账户交互会话、Android `MAR-TL00`/EMUI10 走 adb）；设计级约束（`snapshot_id`、WM 前置、服务必须在会话内、Pillow+xdotool、Windows DPI 感知、Android 坐标同空间）；设计稿 `work/A/checkpoint/spec-screen-1.md`（**§0.1 术语与形态**）；原型 `work/A/checkpoint/screen-lab/`；实测 `checkpoint-3.md`/`checkpoint-4.md`/`checkpoint-5.md`；**术语定案**=`computer` 工具三能力面 term/fs/**graphics**，agent 侧=图形客户端、目标侧=**图形服务**（目标侧不叫 agent；sshd/sftp 保留、只自建图形服务；统一在 agent 侧）；交接 `handoff-screen-04.md`（**下一步=Phase 2 形态讨论**：可安装/自启/远程/**鉴权**、四平台顺序、Android 设备内 app）。**多裁决未定**（图 lineage／自建 vs E2B／客户端厚度／**代码放哪**／Wayland／a11y tree）→ `entries/2026-09-20-cogos-screen-face.md`
  ...
  - **大图慢的根因 = VBox 7.2.x 桥接收包 bug**（非 screenlab/协议：下载 ~64KB/s 且卡死、`rx_errors` 随传输涨、NAT 口正常）；**改用 Tailscale 绕行**——实测同走 enp0s8 仍 4MB/s（原始 TCP 同条件 16.4s/1MB vs tailnet 0.74s），故无需宿主关 RSC → `entries/2026-09-20-cogos-screen-net-vbox-bridge.md`
   ...
   - **#10~#37（09-22~24，P2 多账户 + 装配）**：`screenlab` 三面收敛（管理 `install-machine`/`add-agent`/`remove-agent` · 会话 `open`/`close [--destroy]` · 观察 `view` 只读）+ 使用面 `surface run/windows/focus/close/clip`；每 agent 一账户一**自管 headless Xvfb 面**（systemd user unit 组，"开一个程序"式按需开/关、状态放磁盘/浏览器 profile 按账户复用）；§0.0 目标拉通（唯一约束）、授权=给账户。**#37 本轮按"怎么使用"重验图形面**（不用仓库 e2e 自证，改用 agent 身份 + `import` 抓屏做真值），装配/闭环/生命周期/profile 落盘/观察面/隔离/痕迹均过，定位 F1–F9 → 交接 **`work/A/checkpoint/handoff-screen-37.md`** + 证据底稿 `work/A/checkpoint/tmp-screen-usage-verify.md`
     - **#37 续（本会话）**：已修「纯 bug」批（F1 面未起挂死→存活预检+timeout；F1b alloc_display 查 TCP 6000+N；F2 空剪贴板+xclip 泄漏 stdio；session.env 缩进）+「F6/F9/F7 close 与 GUI 进程」（`surface run` 走 `systemd-run --user --slice=screenlab-apps.slice` 组进程、`surface close` 改真 WM_DELETE、`screenlab close` 温和默认按 cgroup 存活判定、`--force` 收敛）；靶机验证过，**代码未提交**。待 YZ：F8 剩余口径 / F4 是否改 spec 承认仅像素。规则已抽成稳定文件 `work/A/checkpoint/screenlab-rules.md`；交接 **`work/A/checkpoint/handoff-screen-38.md`**。**#38 续（09-24）= 修 F1/F1b/F2 + 实现 F6/F9/F7**（`surface run` 归 `systemd-run --slice=screenlab-apps.slice`；`screenlab close` 温和默认+`--force`，先 WM_DELETE 有界等再按 cgroup 收；`surface close` 改真 WM_DELETE），靶机验证过、**未提交**；待 YZ 裁决 F8/F4 → 交接 `work/A/checkpoint/handoff-screen-38.md`、规则 `work/A/checkpoint/screenlab-rules.md`
