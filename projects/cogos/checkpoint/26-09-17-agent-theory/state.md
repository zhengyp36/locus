...
- **执行**：✅ 齐（模型 + 动作循环 + 工具 + 终端）。
- **触发**：🟡 起步——S3 已实现"启动即跑：壳自动从议程选条并推进"（`loop.pick_next`）；**缺常驻/定时（agent 自己醒来）**。
- **验证**：🟡 壳有独立验收（跑 `acceptance` 退出码，S2/S3 已用）；**缺 agent 内自验证**，且"验收太弱会空转"（S3 教训，对策=先放红测试）。
- **议程**：🟡 有机器可读 `agenda.yaml`（S2/S3 用）；**缺"下一步从哪来"的自动来源**（现仍靠人维护文件）。
- **记忆**：🟡 有进程内 context + scratch + timer 持久；**缺跨会话持久**。
- **感知/视觉**：🟡 库完成，**未接入 consciousness 工具集**（vision not wired）。

**结论**：L1→L2 的门槛（机器可读议程 / 自触发 / agent 内自验证）里，**自触发已起步（S3 启动即跑）**；议程仍是人写文件、自验证仍缺席。缺三格中：`触发` → 半成（仅启动即跑），`议程`、`验证` → 仍缺。

**09-11 续（自驱语义收敛）**：驱动力 = **议程更新函数 `f`**（`A_{n+1}=f(A_n,结果)`），静态议程不驱动、ΔA 才驱动；自主度 = agent 在 `f` 里的占比。分水岭 = **议程为空时**：停（`no_work`）= 自动；自己长出议题 = L3 自驱。新增缺口：**工位隔离**（editable 钉 A + `~/.cogos` 硬编码/服务单例）→ 见 `locus/projects/cogos/ISSUES.md`。详见 `checkpoint-2.md`。

## 三、开放议程候选（S2 首跑**不从中取**，见 §四）

来自 `locus/projects/cogos/ISSUES.md`（按"自包含 + 可验证"排序）。**整张没有"最小 + 可自动验收 + 无需决策"的项**，故首跑另造最小可验靶子：

1. `load_bot` 与 `AccountRef.ensure` 分层错位 —— 边界较清楚，但通信层已收口、跨调用点重构，验收偏虚（首跑**不取**，留后续真实现）。
2. sessions 软链接：群改名同步未实现。
3. resume 重建账号与 setup 的字段差异。
4. 账号失效机制两处遗留（fail-open 续命 / 无 revoke）。
5. 坐标规则注入：静态前置 → 随图动态注入（已定方向，暂缓）。

## 四、YZ 验收（2026-09-11，已裁决）

- ✅ **表认可**（含"回路角色"归位）。
- ✅ **S1 就按** `议程 → 触发 → 执行 → 验证 → 写回 → 停/问`，用**已有件**填。
- ❌ **首条议程不取候选 1**。候选 1 是通信层（已收口）的跨调用点重构，验收偏虚、易跑偏；其余候选要么需决策、要么验收不明。→ 改为 **首跑用"最小可验靶子"**（最小、可自动验收、无需决策，可为候选 1 的最小子切片或临时造一条），目的是先把壳跑通，真实现留后。
- S1 处置：spec 已落笔 `cogos/docs/design-selfdrive-loop-s1.md`。
