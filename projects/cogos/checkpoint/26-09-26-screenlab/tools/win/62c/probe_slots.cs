...
class ProbeSlots
{
    ...
    static void Main(string[] args)
    {
        if (args.Length >= 2 && args[0] == "child")
        {
            ...
            int hr = d(outp, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero);
            Console.WriteLine("ctx|" + idx + "|0x" + ((uint)hr).ToString("X8"));
            return;
        }

        IntPtr o = GetOutput();
        Console.WriteLine("ptrsize|" + IntPtr.Size + "|output|0x" + o.ToString("X")
            + "|vtable|0x" + Marshal.ReadIntPtr(o).ToString("X"));
