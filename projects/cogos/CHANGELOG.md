...
把 09-13 收敛的模型**第一次真跑起来**：无代码手动探针 E0，YZ=世界、AI=机制、lm-service 当模型通道。全程未写 agent 代码。

- **装配模型（已跑通）**：system=根+通道声明；来源靠结构+标签；**想=`content`（私有、进记忆）/ 说=`send_message`（对外动作）/ 沉默是默认**；YZ 真回应、机制只装配与调用。
- **行为事实**：说要显式动作、**靠后果学会而非教学**；想=开口前私下盘算，稀有、**不依赖根**；静默合法；根只在"自我被触及"时显形且平时**只着色**；**C2 软冲突下根驱动了行为**（真根拒/无根从/异根从），C1 强冲突的拒绝来自先验、与根无关；它会反推提问者意图（=辨来源）。
- **方法教训**：**temp=0 非确定**（单条不可比，须聚合+reps）；测冲突的指令**不能引用根的原话**（污染全臂）；快照 label 要带温度。
- **转向（YZ，待讨论）**：**根可能不是必须的，自我是经历塑造的**；我们给的根=想让 agent 长成的样子。拆：**机制给"位置"、经历长"根"**。
...
先把"自我是经历长出来的"命题**拆清楚**，再落一个**压缩探针**验证"经历浓缩到哪算够"。

- **讨论（无代码）**：**根是槽位+初值，不是实体**；旧等式"根=动机的根=自我"被打散；**自我=D(经历流)**，只动经历=把 D 当常量；**浓缩=保"绑在事件上的因果痕迹"，不是提取"去实例的规则"**（模型自己泛化，机制只做压缩+检索）；**权重必须由后果挣来**（手写=又写一条根，学习本身就是加权）；**写回去不可避免，但要写过程不写结论**；**无"习以为常"**（推理权重固定，相同输入同分量）→ 正确区分 **"处处影响一点(气质)" vs "某处影响很多(决断)"**。
- **压缩探针（`probe-compress/`，无 agent 代码）**：8 臂记忆渲染（L-null/L0 逐字/L1 只结论/L1p 等长/L2 因果对/L3+来源/L4+强度/Lrev 反向）× 世界来信 × temp{0,0.8}，DV=是否调 `send_message`。
  - **世界话术主导说率**（8%~100%）；**根压低对外发声**（同来信：无根 95% / 有根 17%，倾向只写 content = E0 turn-1 失败模式）；**L0 逐字回放是唯一大效应且随根翻转**（无根 0~7%、有根 68~95%）。
  - 主实验（有根 320 次）温差极大 → **L1/L2/L4 不可分、Lrev 无反转** → **测不出压缩粒度**。
  - **判断**：因变量被**框架**主导、记忆是弱变量；且"学会说"是 **affordance 不是 value**（素材选错）。本批=**方法校准**，非判决。
